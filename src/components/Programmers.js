...
export default function Programmers() {
  ...
  const getNameOfFeatured = () => {
    ...
    let featuredName = programmers.filter(item => {
      if(item.id === featuredId){
        return(item.name)
      }
    })
    console.log('featured =', featuredName[0].name)
    return featuredName[0].name
  };
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => { /* in here set the featured id to be dev.id */ setFeaturedId(dev.id) }}>Feature</button>
            </div>
          )
        }
      </div>
